...
func TestLinkNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.BrowserRendering.Links.New(context.TODO(), browser_rendering.LinkNewParams{
		...
		Body: browser_rendering.LinkNewParamsBodyObject{
			...
			Authenticate: cloudflare.F(browser_rendering.LinkNewParamsBodyObjectAuthenticate{
				Password: cloudflare.F("x"),
				Username: cloudflare.F("x"),
			}),
			BestAttempt: cloudflare.F(true),
			Cookies: cloudflare.F([]browser_rendering.LinkNewParamsBodyObjectCookie{{
				...
				Priority:     cloudflare.F(browser_rendering.LinkNewParamsBodyObjectCookiesPriorityLow),
				SameParty:    cloudflare.F(true),
				SameSite:     cloudflare.F(browser_rendering.LinkNewParamsBodyObjectCookiesSameSiteStrict),
				Secure:       cloudflare.F(true),
				SourcePort:   cloudflare.F(0.000000),
				SourceScheme: cloudflare.F(browser_rendering.LinkNewParamsBodyObjectCookiesSourceSchemeUnset),
				URL:          cloudflare.F("url"),
			}}),
			EmulateMediaType:     cloudflare.F("emulateMediaType"),
			ExcludeExternalLinks: cloudflare.F(true),
			GotoOptions: cloudflare.F(browser_rendering.LinkNewParamsBodyObjectGotoOptions{
				Referer:        cloudflare.F("referer"),
				ReferrerPolicy: cloudflare.F("referrerPolicy"),
				Timeout:        cloudflare.F(60000.000000),
				WaitUntil:      cloudflare.F[browser_rendering.LinkNewParamsBodyObjectGotoOptionsWaitUntilUnion](browser_rendering.LinkNewParamsBodyObjectGotoOptionsWaitUntilString(browser_rendering.LinkNewParamsBodyObjectGotoOptionsWaitUntilStringLoad)),
			}),
			RejectRequestPattern: cloudflare.F([]string{"string"}),
			RejectResourceTypes:  cloudflare.F([]browser_rendering.LinkNewParamsBodyObjectRejectResourceType{browser_rendering.LinkNewParamsBodyObjectRejectResourceTypeDocument}),
			SetExtraHTTPHeaders: cloudflare.F(map[string]string{
				"foo": "string",
			}),
			SetJavaScriptEnabled: cloudflare.F(true),
			UserAgent:            cloudflare.F("userAgent"),
			Viewport: cloudflare.F(browser_rendering.LinkNewParamsBodyObjectViewport{
			...
			VisibleLinksOnly: cloudflare.F(true),
			WaitForSelector: cloudflare.F(browser_rendering.LinkNewParamsBodyObjectWaitForSelector{
				Selector: cloudflare.F("selector"),
				Hidden:   cloudflare.F(browser_rendering.LinkNewParamsBodyObjectWaitForSelectorHiddenTrue),
				Timeout:  cloudflare.F(120000.000000),
				Visible:  cloudflare.F(browser_rendering.LinkNewParamsBodyObjectWaitForSelectorVisibleTrue),
			}),
